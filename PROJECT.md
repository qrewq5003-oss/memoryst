> **Статус: это исходный замысел от 2026-03-15, а не описание текущей системы.**
> Документ сохранён как история решений. Где он разошёлся с реальностью:
> - сервис работает **не на Steam Deck**, а в Termux на том же Android-устройстве, что и ST;
> - извлечение — **не rule-based**: основной путь через LLM со structured output
>   (`scene_extractor.py` + `llm_extractor.py`), regex остался фоллбэком;
> - API вырос далеко за перечисленные семь эндпоинтов (трекеры, сцены, аудит, бэкфилл,
>   консолидация, версия, провайдеры);
> - к слоям памяти добавились `summary` и `tracker`, а рядом с ними — сырая история
>   в `chat_messages` + FTS5.
>
> Актуальное состояние архитектуры — в `CLAUDE.md`, открытые проблемы —
> в `docs/extension_audit_2026-09-20.md`.
