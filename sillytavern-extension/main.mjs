...
} from './audit.mjs?v=f9f767d';
import {
    normalizeExtensionSettings,
    serializeExtensionSettings,
} from './settings.mjs?v=f9f767d';
import { mountSettingsUi } from './settings-ui.mjs?v=f9f767d';
import { resolveEffectiveScope } from './scope.mjs?v=f9f767d';
import {
    buildLoreAnchorBlock,
    LORE_ANCHOR_PROMPT_KEY,
} from './lore-anchors.mjs?v=f9f767d';
import {
    buildTrackerBlock,
    ...
    TRACKER_PROMPT_KEY,
} from './trackers.mjs?v=f9f767d';
import {
    MEMORY_EXTENSION_BUILD,
    MEMORY_PROTOCOL_VERSION,
    compareVersions,
} from './version.mjs?v=f9f767d';
import {
    findEnumDrift,
...
} from './supersede.mjs?v=f9f767d';
import {
    summarizeForeignInjectors,
    summarizeWorldInfo,
} from './injectors.mjs?v=f9f767d';
import {
...
function injectTrackersFor(entries = []) {
    ...
    const diagnostics = {
        ...
        entryCount: (entries || []).length,
        // The comment of each activated entry - that is where an @memory-tracker marker
        // lives, so when the marker "does not work" this says whether the payload even
        // carried it. Comments only: dumping every field of every entry into the audit was
        // forensics for one specific question, not something worth writing every turn.
        entryComments: (entries || []).slice(0, 4).map(entry => String(entry?.comment ?? '').slice(0, 80)),
    };
